...
class LoanView(APIView):
    ...
    def get(self, request) -> Response:
        ...
        serializer: LoanResponseSerializer = LoanResponseSerializer(
            instance=loans, many=True
        )
...
class LoanDetailView(APIView):
    ...
    def patch(self, request, **kwargs):
        ...
        """
        loan: Loan = Loan.objects.filter(
            loan_id=kwargs.get("loan_id"), is_returned=False
        ).first()
        if not loan:
            raise NotFound("Invalid loan id or This loan has already been returned")

        serializer: LoanUpdateSerializer = LoanUpdateSerializer(
